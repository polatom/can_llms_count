...
Ported verbatim from experiment_01/src/conllu_utils.py (validated there with
zero text-reconstruction mismatches). Stdlib-only. The one non-trivial piece is
`assign_word_indices`, which maps each syntactic token to a **whitespace-word**
index (the unit the distance metric is defined in — see METHODOLOGY.md §3.4),
collapsing Czech multiword tokens (e.g. ``aby`` -> ``aby`` + ``by``) into a
single surface word.
"""
