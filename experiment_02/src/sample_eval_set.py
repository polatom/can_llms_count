...
DEFAULT_KUK_ROOT = os.path.normpath(
    os.path.join(
        ...
        "..", "..", "experiment_01", "data", "raw", "KUK_1.0", "KUK_1.0", "data",
    )
)
MAX_WORDS = 200
...
def enumerate_sentences(files: list[str], root: str):
...
def write_audit(out_md: str, n: int, seed: int, stats: Counter, per_subcorpus: Counter,
                sample: list[dict], extractions: list[dict]) -> None:
    lines: list[str] = []
    w = lines.append
    ...
    exp140 = round(verdicts / n_sent * 140) if n_sent else 0
    w(f"- expected positives in the random gold-140: **≈ {exp140}** "
...
def main() -> None:
    ...
    kept, stats, per_subcorpus = enumerate_sentences(files, args.kuk_root)
