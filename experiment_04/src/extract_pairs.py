...
_REL_LEMMAS = {"který", "jenž", "kterýžto", "jaký", "což"}


def _is_rel_pron(t: Token) -> bool:
    """Relative-pronoun subject (K8 flag).

    Primary signal: PronType=Rel. Fallback for treebanks with sparse FEATS
    (the converted cs_cltt): PRON/DET with a relative-pronoun lemma, or the
    PDT XPOS relative-pronoun tags (P4/P9/PJ/PK).
    """
    if "PronType=Rel" in t.feats:
        return True
    if t.upos in ("PRON", "DET"):
        if t.lemma.lower() in _REL_LEMMAS:
            return True
        if t.xpos[:2] in ("P4", "P9", "PJ", "PK"):
            return True
    return False
